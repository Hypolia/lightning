import { ConditionContract, valVerite, Prop } from '@lightning/board/filter/types'
import { Formule } from '@lightning/board/filter/logic'
...
export class Logic<T> {
   ...
   * @param data
   */
  public execute (data: T): valVerite {
    const fn = (prop: Prop): valVerite => {
      switch (prop.kind) {
        case "and":

import {ConditionContract, Prop, RowCondition, SymbContract} from '@lightning/board/filter/types'
...
function isFilterData<T> (node: ConditionContract<T> | RowCondition<T>): node is ConditionContract<T> {
...
export class Formule<T> {
  ...
  public getCondition (): ConditionContract<T> {
  ...
  public affiche (): string {
    ...
    function fn (fbf: Prop, symb: Prop): string {
